...
if (iconMenu) {
  ...
  iconMenu.addEventListener('click', function (e) {
    ...
    menuBody.classList.toggle('_active'); // Добавляем для Меню класс "active"
  });
}
...
if (menuLinks.length > 0) {
  ...
  function onMenuLinkClick(e) {
    ...
    if (menuLink.dataset.goto && document.querySelector(menuLink.dataset.goto)) {
      // Проверяем заполнен ли дата атрибут "data-goto" && Проверяем, существует ли объект, на который ссылается этот артибут
      ...
      if (iconMenu.classList.contains('_active')) {
        // Если Бургер содержит класс "active"
      ...
      window.scrollTo({
        // Скролл к - "top", как? - "smooth (плавно)"
        top: gotoBlockValue,
        behavior: 'smooth',
      });
      e.preventDefault(); // Выключает перезагрузки страницы при клике на ссылку
    }
  }
}
...
audio.src = `audio/${audioLinks[0].innerText}.mp3`;

audioLinks.forEach((element) => {
  element.addEventListener('click', function () {
    if (!this.classList.contains('_active')) {
      audio.src = `audio/${this.innerText}.mp3`;
      audioLinks.forEach((element) => {
